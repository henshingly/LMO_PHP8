...
else $spieler_ligalink = $text['spieler'][11];
...
if (isset($file) && $file != "")
{
    ...
    if ($_SESSION['lmouserok'] == 2 || ($_SESSION['lmouserok'] == 1 && $spieler_adminbereich_hilfsadmin_zulassen == 1))
    {
        ...
        if (isset($_REQUEST['nullwerte_anzeigen'])) $spieler_nullwerte_anzeigen = 1;
        else
        {
            if ($spieler_option == "saveconfig") $spieler_nullwerte_anzeigen = 0;
        }
        if (isset($_REQUEST['vereinsweise_anzeigen'])) $spieler_vereinsweise_anzeigen = 1;
        else
        {
            if ($spieler_option == "saveconfig") $spieler_vereinsweise_anzeigen = 0;
        }
        if (isset($_REQUEST['extra_sortierspalte'])) $spieler_extra_sortierspalte = 1;
        else
        {
            if ($spieler_option == "saveconfig") $spieler_extra_sortierspalte = 0;
        }
        if ($_SESSION['lmouserok'] == 2)
        {
            if (isset($_REQUEST['adminbereich_hilfsadmin_zulassen'])) $spieler_adminbereich_hilfsadmin_zulassen = 1;
            else
            {
                if ($spieler_option == "saveconfig") $spieler_adminbereich_hilfsadmin_zulassen = 0;
            }
            if (isset($_REQUEST['adminbereich_hilfsadmin_fuer_spalten'])) $spieler_adminbereich_hilfsadmin_fuer_spalten = 1;
            else
            {
                if ($spieler_option == "saveconfig") $spieler_adminbereich_hilfsadmin_fuer_spalten = 0;
            }
        }
...
<?php
    ...
    function cmpStr($a2, $a1)
    {
        ...
        if (!$c) $c = strnatcasecmp($a1[$spieler_sort], $a2[$spieler_sort]);
        return $c;
    }
