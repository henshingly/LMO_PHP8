...
if ($filepointer = fopen($filename, 'r+b')) {
    ...
    if ($formel)
        fgetcsv($filepointer, 10000, '#');  // Zeile mit Formeln
    ...
    array_pop($data);
    if ($spieler_nullwerte_anzeigen == 0 && !isset($typ[$sort]))
        $data = array_filter($data, 'filterNullwerte');  // Nullwerte ausfiltern
    if ($direction == 1) {
        if (!isset($typ[$sort]))
            usort($data, 'cmpInt');
        else
...
?><td align="right"><strong><?php
                if (!isset($data[$j1 - 1][$sort]) || $data[$j1][$sort] !== $data[$j1 - 1][$sort] && $j1 != $begin)
                    echo ($j1 + 1) . '. ';
