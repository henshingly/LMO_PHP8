  ...
  <body><?php
    ...
    if ($file && $liga->loadFile(PATH_TO_LMO . '/' . $dirliga . $file) == TRUE) {
        ...
        if (isset($partie)) {
            ...
            $w = intval($mini_barWidth * $winCount / ($spAnzahl + 0.1));
            $d = intval($mini_barWidth * $drawCount / ($spAnzahl + 0.1));
            $l = intval($mini_barWidth * $lostCount / ($spAnzahl + 0.1));
        ...
        echo getMessage($text['mini'][5], TRUE);
    // Falls IFRAME - komplettes HTML-Dokument
    if (basename($_SERVER['PHP_SELF']) == 'lmo-mininext.php') {
?>
