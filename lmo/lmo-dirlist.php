        ...
        if ($datei && check_hilfsadmin($files)) {
            ...
            while (!feof($datei)) {
                ...
                if ((substr($zeile, 0, 1) == '[') && (substr($zeile, -1) == ']')) {  // Sektion
                    ...
                    if ($option_name == 'Actual') {
                        $ligadatei[$liga_counter]['aktueller_spieltag'] = $option_wert;
                    }
                    if ($option_name == 'Teams') {
                        $ligadatei[$liga_counter]['anz_teams'] = $option_wert;
                    }
                    if ($option_name == 'icon') {
                        $ligadatei[$liga_counter]['icon'] = $option_wert;
                    }
                    if ($option_name == 'Type') {
                ...
                if ($ligadatei[$liga_counter]['rundenbezeichnung'] != $text[2]) {  // Pokal
                    ...
                    switch ($ligadatei[$liga_counter]['aktueller_spieltag']) {
                        case ($t5):
                            ...
                            break;
                        case ($t5 - 1):
                            $countTeams == 24 ? $ligadatei[$liga_counter]['rundenbezeichnung'] = $text[373] : $ligadatei[$liga_counter]['rundenbezeichnung'] = $text[374];
                            break;
                        case ($t5 - 2):
                            $countTeams == 24 ? $ligadatei[$liga_counter]['rundenbezeichnung'] = $text[372] : $ligadatei[$liga_counter]['rundenbezeichnung'] = $text[373];
                            break;
                        case ($t5 - 3):
...
if (isset($_SESSION['liga_sort_direction']) && $_SESSION['liga_sort_direction'] == 'desc')
    $ligadatei = array_reverse($ligadatei);
?>
...
if ($liga_counter == 0) {
    echo "<div class='col'>[" . $text[223] . ']</div>';
}
?>
</div>
<?php
